...
|--------|------|----------|--------------------------|
| Hyde Park Ave | 1,069 - 29,273 | 2-4 | 19 |
| Washington St | 1,154 - 41,782 | 1-5 | 87 |
| Cummins Hwy | 27,261 | 2-4 | 5 |
| Centre St | 505 - 29,065 | 1-4 | 48 |
...
1. **[MassDOT Traffic Inventory 2024](https://geo-massdot.opendata.arcgis.com/datasets/MassDOT::traffic-inventory-2024)**: 438K road segments with AADT, lane count, road classification. CSV download.

2. **[MassDOT Bike Level of Traffic Stress](https://geo-massdot.opendata.arcgis.com/datasets/MassDOT::bike-level-of-traffic-stress)**: 608K road segments with LTS score, speed limit, surface width, shoulder type. CSV download. Methodology: [Mekuria, Furth & Nixon (2012)](https://transweb.sjsu.edu/research/low-stress-bicycling-and-network-connectivity).

3. **[Boston Vision Zero Crash Records](https://data.boston.gov/dataset/vision-zero-crash-records)**: 42K crash records (2015-2025) with timestamp, lat/lon, and mode (bike/car/pedestrian). CSV download. Updated monthly.

4. **[OpenStreetMap](https://www.openstreetmap.org/)** via [OSMnx](https://osmnx.readthedocs.io/) ([Boeing, 2017](https://doi.org/10.1016/j.compenvurbsys.2017.05.004)): Road network graph for the study area. 5,786 intersections, 13,635 road segments.
...
$$\mathbf{h}_v^{(l+1)} = \sigma\left(\mathbf{W}^{(l)} \cdot \text{CONCAT}\left(\mathbf{h}_v^{(l)},\; \text{AGG}\left(\left\{\mathbf{h}_u^{(l)} : u \in \mathcal{N}(v)\right\}\right)\right)\right)$$
...
$$\hat{p}_v = \sigma\left(\mathbf{w}_c^\top \mathbf{h}_v^{(3)} + b_c\right) \quad \text{(crash probability)}$$

The loss combines MSE for regression and weighted binary cross-entropy for classification:

$$\mathcal{L} = \frac{1}{|V_{\text{train}}|}\sum_{v \in V_{\text{train}}} \left(y_v - \hat{y}_v^{\text{reg}}\right)^2 - \frac{1}{|V_{\text{train}}|}\sum_{v \in V_{\text{train}}} \left[w^+ y_v^{\text{bin}} \log \hat{p}_v + (1 - y_v^{\text{bin}}) \log(1 - \hat{p}_v)\right]$$
